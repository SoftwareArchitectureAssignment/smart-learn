  ...
  useEffect(() => {
    if (content && open) {
      ...
      if (content.type === "VIDEO" && content.video) {
        setVideoUrl(content.video.url);
        setVideoDuration(content.video.duration?.toString() || "");
      } else if (content.type === "DOCUMENT" && content.document) {
        setDocumentUrl(content.document.url);
      } else if (content.type === "TEXT" && content.text) {
        setTextBody(content.text.body);
      } else if (content.type === "QUIZ" && content.quiz) {
        setPassingScore(content.quiz.passingScore?.toString() || "70");
        if (content.quiz.questions && content.quiz.questions.length > 0) {
          setQuestions(
            content.quiz.questions.map((q: any) => ({
              id: q.id,
              text: q.text,
