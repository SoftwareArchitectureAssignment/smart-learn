...
const TakeTest = () => {
  ...
  // Mock questions - replace with actual API call based on topic and difficulty
  const questions: TestQuestion[] = Array.from({ length: config?.numberOfQuestions || 20 }, (_, i) => ({
    id: `${i + 1}`,
    question: `Question ${i + 1}: In ${config?.topic}, which of the following best describes the main concept?`,
    options: [
      "Option A: First possible answer that might be correct",
      "Option B: Second possible answer to consider",
      "Option C: Third option with different approach",
      "Option D: Fourth alternative explanation",
    ],
  ...
  const handleSubmitTest = () => {
    ...
    navigate("/student/placement-test/result", {
      state: {
        score,
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{config.topic} Assessment</h1>
          <p className="text-gray-500">
            Question {currentQuestion + 1} of {questions.length} • {config.difficulty} Level
          </p>
        </div>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex items-center gap-4 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              <div className="size-4 rounded border border-gray-300 bg-gray-100" />
              <span>Not answered</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="size-4 rounded border border-green-300 bg-green-100" />
              <span>Answered</span>
            </div>
            <div className="flex items-center gap-2">
